...
export function readBytes (source: Editor, size?: number): number[] {
    const tokens = source.getValue().trim().split(/\s+/u);
    return zip(range(size! || tokens.length), tokens, (_: number, x): number => {
        return ((x || '').startsWith('0b') ? parseInt(x.slice(2), 2) : Number(x)) || 0;
    });
}
export function writeBytes (target: Editor, size: number | null, bytes: ArrayBuffer | number[]): void { // TODO is ArrayBuffer | number[] correct?
    target.setValue(zip(range(size || (bytes as number[]).length), bytes as number[], (_: number, x: number): string => hexFormat(x & 0xFF)).join(' '));
}
...
export function drawOnCanvas (target: HTMLCanvasElement, body: (x: number, y: number, draw: boolean) => void): void {
    let mode = 0;
    ...
    function release (event: MouseEvent): void { mode = 0; drag(event); }
    function press (event: MouseEvent): void { mode = event.button == 2 ? 2 : 1; drag(event); }
    function context (event: MouseEvent): boolean { drag(event); return false; }
...
export function setVisible (element: HTMLElement, value: boolean, disp?: string): void {
...
export function textBox (element: HTMLElement, readonly: boolean, value: string): CodeMirror.Editor {
