...
export function unpackOptions(
  emulatorToUnpack: Emulator,
  options: EmulatorOptions
): void {
  optionFlags.forEach((x: string): void => {
    if (x in options) emulatorToUnpack[x] = options[x];
  });
...
export function getColor(id: number): string {
  switch (id) {
    case 0:
      return emulator.backgroundColor;
    case 1:
      return emulator.fillColor;
    case 2:
      return emulator.fillColor2;
    case 3:
      return emulator.blendColor;
    default:
...
export function renderDisplay(emulatorToRender: Emulator): void {
  ...
  const colors = [
    emulatorToRender.backgroundColor,
    emulatorToRender.fillColor,
    emulatorToRender.fillColor2,
    emulatorToRender.blendColor
  ];
  if (c.last !== undefined) {
    if (
      arrayEqual(c.last.p[0], emulatorToRender.p[0]) &&
      arrayEqual(c.last.p[1], emulatorToRender.p[1]) &&
      arrayEqual(c.last.colors, colors)
    ) {
      return;
    }
    if (c.last.hires !== emulatorToRender.hires) {
      c.last = undefined;
    } // full redraw when switching resolution
  }
  ...
  for (let y = 0; y < h; ++y) {
    for (let x = 0; x < w; ++x, ++z) {
      ...
      const colorIdx =
        emulatorToRender.p[0][z] + (emulatorToRender.p[1][z] << 1);
  ...
  if (audio && !audioNode) {
    ...
    audioNode.onaudioprocess = function(
      ...
      while (audioData.length && index < samplesN) {
        ...
        // TODO fix this cast
        const written = audioData[0].write(
          (outputData as unknown) as number[],
          index,
          size
        );
        index += written;
      ...
      if (audioData.length > 1) {
        ...
        audioData.forEach(function(buffer): void {
          audioDataSize += buffer.duration;
        });
...
export function playPattern(
  ...
  if (remainingTicks && audioData.length > 0) {
    audioData[audioData.length - 1].dequeue(
      Math.floor((remainingTicks * audio.sampleRate) / TIMER_FREQ)
    );
  }
  ...
  audioData.push(
    new AudioBuffer(
      audioBuffer,
      Math.floor((soundLength * audio.sampleRate) / TIMER_FREQ)
    )
  );
}
