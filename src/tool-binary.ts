...
const binaryEditor = textBox(
  document.getElementById("binary-editor")!,
  false,
  ""
)!;
const decompilerMode = radioBar(
  document.getElementById("decompiler-mode")!,
  "static",
  (): void => {}
)!; // TODO unused x?
radioBar(
  document.getElementById("decompiler-numbers")!,
  Formats.hex,
  (x: Formats): void => {
    emulator.numericFormatStr = x;
  }
);
...
function decompileRaw(rom: number[]): void {
  ...
  for (let x = 0; x < rom.length; x += 2) {
    ...
    const b = rom[x + 1] | 0;
    r +=
      "\t" + hexFormat(a) + " " + hexFormat(b) + " # " + hexFormat(0x200 + x);
...
document.getElementById("binary-decompile")!.onclick = (): void => {
  (decompilerMode.getValue() == "static" ? decompileStatic : decompileRaw)(
    readBytes(binaryEditor)
  );
};
...
document.getElementById("binary-save-ch8")!.onclick = (): void => {
  ...
  const name = (document.getElementById("binary-filename") as HTMLInputElement)
    .value;
  saveAs(
    new Blob([new Uint8Array(prog.rom)], { type: "application/octet-stream" }),
    name + ".ch8"
  );
};
...
document.getElementById("binary-save-cart")!.onclick = (): void => {
  ...
  saveAs(
    new Blob([new Uint8Array(cart)], { type: "image/gif" }),
    name + ".gif"
  );
};

writeBytes(binaryEditor, null, [
  0xd0,
  0x15,
  0x70,
  0x04,
