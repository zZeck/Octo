...
function dumpRegisters(showV: boolean, name: string): string {
  ...
  const line = (text: string, click?: string): string =>
    "<span" +
    (click ? ' onClick="' + click + '"' : "") +
    ">" +
    text +
  ...
  const register = (
    n: string,
    v: number,
    f: (addr: any, reg?: any, raw?: any) => string
  ): string =>
    line(
      n + " := " + numericFormat(v, regNumFormat[n || "hex"]) + " " + f(v, n),
      "cycleNumFormat('" + n + "')"
    );
  const aliases = (_addr: number, reg: number[]): string => {
    // TODO fix addr?
    const a = emulator.metadata.aliases;
    const r = Number(`0x${reg.slice(1)}`);
    const n = Object.keys(a)
      .filter((k: string): boolean => a[k] == r)
      .join(", ");
    return n.length ? "(" + n + ")" : "";
  };
  return (
    ...
    (showV
      ? range(16)
          .map((x: number): string =>
            register("v" + x.toString(16).toUpperCase(), emulator.v[x], aliases)
          )
          .join("")
      : "") +
    "<br>"
...
function dumpStack(): string {
  return (
    "inferred stack trace:<br>" +
    emulator.r
      .map((x: number): string => hexFormat(x) + getLabel(x) + "<br>")
      .join("") +
    "<br>"
  );
}
...
function dumpContext(): string {
  ...
  let memlo = emulator.pc;
  let memhi = emulator.pc;
  while (dbg!.getLine(memlo - 1)! > pcline - 8) memlo--;
  while (dbg!.getLine(memhi + 1)! < pcline + 8) memhi++;
  let ind = memlo;
  const lines = [];
  for (let x = dbg!.getLine(memlo)!; x <= dbg!.getLine(memhi)!; x++)
    lines.push(x);
  const row = (c: boolean, a: string, d: string, s: string): string =>
    "<tr" +
    (c ? " class='current'" : "") +
    "><td>" +
    a +
    ...
    escapeHtml(s) +
    "</pre></td></tr>";
  const linebytes = (x: number): string => {
    let r = "";
    while (dbg!.getLine(ind) == x)
      r += hexFormat(emulator.m[ind++]).slice(2) + " ";
    return r;
  };
  return (
    ...
    lines
      .filter((x: number): boolean => !/^\s*$/u.exec(dbg!.lines[x]))
      .map((x: number): string => {
...
function dumpProfile(): string {
  ...
  for (let addr = 0; addr < 65536; ) {
    ...
    const head = addr;
    const label = getLabel(addr, 0, true);
    let ticks = 0;
    while (
      emulator.profileData[addr] != undefined &&
      getLabel(addr, 0, true) == label
    ) {
  ...
  return (
    ...
    profile
      .sort((a, b): number => b.percent - a.percent)
      .slice(0, 20)
      .map(
        (x: {
          ticks: number;
          ...
          `<tr><td>${x.ticks}</td> <td>${x.percent.toFixed(2)}%</td> <td>${
            x.calls
          }</td> <td>${x.source}</td></tr>`
      )
      .join("") +
    "</table>" +
    "<div class='debug-profile-results'>" +
    "<div>Full results:<div>" +
    "<div class='debug-profile-full'>" +
...
export function haltBreakpoint(name?: string): void {
  ...
  debugPanel!.innerHTML =
    dumpRegisters(true, name!) + dumpStack() + dumpContext();
}
