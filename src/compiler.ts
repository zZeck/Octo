...
function hexFormat(n: number): string {
...
export class DebugInfo {
  ...
  public getAddr(line: number): number | undefined {
    for (const addr in this._locs) {
      if (this.posToLine(this._locs[addr]) == line)
        return (addr as unknown) as number; // TODO fix
    }
    return undefined;
  }
...
const unaryFunc: { [func: string]: (x: number, m: number[]) => number } = {
  ...
  "@": function(x: number, m: number[]): number {
    return m[(0 | x) - programEntryAddress] || 0;
  }
};
...
export class Compiler {
  ...
  private macros: {
    [name: string]: {
      args: string[];
      body: [stringOrNumber, number, number][];
    };
  }; // map<name, {args, body}>
  ...
  public go(): void {
    ...
    while (!this.end()) {
      if (typeof this.peek() === "number") {
        ...
        if (nn < Int8Min || nn > Int8Max) {
          throw Error(
            `Literal value '${nn}' does not fit in a byte- must be in range [-128, 255].`
          );
        }
    ...
    if (this.branches.length > 0) {
      ...
      throw Error(
        `This '${this.branches[0][2]}' does not have a matching 'end'.`
      );
    }
    for (let index = 0; index < this.rom.length; index++) {
      if (typeof this.rom[index] === "undefined") {
        this.rom[index] = 0x00;
      }
    }
  }
  ...
  private data(a: number): void {
    if (typeof this.rom[this.hereaddr - programEntryAddress] !== "undefined") {
      throw Error(
        "Data overlap. Address " +
          hexFormat(this.hereaddr) +
  ...
  private next(): stringOrNumber {
    this.pos = this.tokens[this.currentToken++];
    return this.pos[0];
  }
  private raw(): stringOrNumber[] {
    this.pos = this.tokens[this.currentToken++];
    return this.pos;
  }
  private peek(): stringOrNumber {
    return this.tokens[this.currentToken][0];
  }
  private here(): number {
    return this.hereaddr;
  }
  private inst(a: number, b: number): void {
    this.data(a);
    this.data(b);
  }
  ...
  private expect(token: string): void {
    ...
    if (thing != token) {
      throw Error(`Expected '${token}', got '${thing}'!`);
    }
  }
  ...
  private constantValue(): number {
    ...
    if (typeof number !== "number") {
      if (number in this.protos) {
        throw Error(
          "Constants cannot refer to the address of a forward declaration."
        );
        ...
        throw Error("Undefined name '" + number + "'.");
      }
    }
    return number;
  }
  ...
  private checkName(name: string, kind: string): string {
    if (name in this.reservedNames || name.startsWith("OCTO_")) {
      throw Error(
        "The name '" +
          name +
          "' is reserved and cannot be used for a " +
          kind +
          "."
      );
    }
  ...
  private shortValue(nn?: number | string): number {
    ...
    // vx:=, vx+=, vx==, v!=, random
    if (!processedByte && processedByte != 0) {
      processedByte = this.next();
    }
    if (typeof processedByte !== "number") {
      if (processedByte in this.constants) {
        processedByte = this.constants[processedByte];
      } else {
        throw Error("Undefined name '" + processedByte + "'.");
      }
    }
    // silently trim negative numbers, but warn
    // about positive numbers which are too large:
    if (
      typeof processedByte !== "number" ||
      processedByte < -128 ||
      processedByte > 255
    ) {
      throw Error(
        `Argument '${processedByte}' does not fit in a byte- must be in range [-128, 255].`
      );
    }
  ...
  private tinyValue(): number {
    ...
    let n = this.next();
    if (typeof n !== "number") {
      if (n in this.constants) {
        n = this.constants[n];
      } else {
        throw Error("Undefined name '" + n + "'.");
      }
    }
  ...
  private conditional(negated: boolean): void {
    ...
    if (token == "==") {
      if (this.isRegister()) {
        this.inst(0x90 | reg, this.register() << 4);
      } else {
        this.inst(0x40 | reg, this.shortValue());
      }
    } else if (token == "!=") {
      if (this.isRegister()) {
        this.inst(0x50 | reg, this.register() << 4);
      } else {
        this.inst(0x30 | reg, this.shortValue());
      }
    } else if (token == "key") {
      ...
      if (this.isRegister()) {
        this.fourop(0x8, compTemp, this.register(), 0x0);
      } else {
  ...
  private controlToken(): [stringOrNumber, number, number] {
    ...
    if (index + this.currentToken >= this.tokens.length) {
      index = this.tokens.length - this.currentToken - 1;
    }
  ...
  private iassign(token: string): void {
    if (token == ":=") {
      const o = this.next();
      if (o == "hex") {
        this.inst(0xf0 | this.register(), 0x29);
      } else if (o == "bighex") {
        this.schip = true;
        ...
        this.immediate(0xa0, this.wideValue(o));
      }
    } else if (token == "+=") {
  ...
  private vassign(reg: number, token: string): void {
    if (token == ":=") {
      const o = this.next();
      if (this.isRegister(o)) {
        this.fourop(0x8, reg, this.register(o), 0x0);
      } else if (o == "random") {
        this.inst(0xc0 | reg, this.shortValue());
      } else if (o == "key") {
        this.inst(0xf0 | reg, 0x0a);
      } else if (o == "delay") {
        this.inst(0xf0 | reg, 0x07);
      } else {
        this.inst(0x60 | reg, this.shortValue(o));
      }
    } else if (token == "+=") {
      if (this.isRegister()) {
        this.fourop(0x8, reg, this.register(), 0x4);
      } else {
        this.inst(0x70 | reg, this.shortValue());
      }
    } else if (token == "|=") {
      this.fourop(0x8, reg, this.register(), 0x1);
    } else if (token == "&=") {
      this.fourop(0x8, reg, this.register(), 0x2);
    } else if (token == "^=") {
      this.fourop(0x8, reg, this.register(), 0x3);
    } else if (token == "-=") {
      this.fourop(0x8, reg, this.register(), 0x5);
    } else if (token == "=-") {
      this.fourop(0x8, reg, this.register(), 0x7);
    } else if (token == ">>=") {
  ...
  private resolveLabel(offset: number): void {
    ...
    if (label in this.dict) {
      throw Error("The name '" + label + "' has already been defined.");
    }
      ...
      for (const addr of this.protos[label]) {
        if (this.longproto[addr]) {
          ...
          if ((target & 0xfff) != target) {
            throw Error(
              `Value '${target}' for label '${label}' cannot not fit in 12 bits!`
            );
          }
          this.rom[addr - 0x1ff] =
            (this.rom[addr - 0x1ff] & 0xf0) | ((target >> 8) & 0xf);
          ...
          this.rom[addr - 0x200] =
            (this.rom[addr - 0x200] & 0xf0) | ((target >> 8) & 0xf);
          this.rom[addr - 0x1ff] = target & 0xff;
        }
      }
  ...
  private parseTerminal(name: string): number {
    ...
    } //TODO replaced +x == +x condition. Is this correct?
    if (x in this.constants) {
      return this.constants[this.next()];
    }
    if (x in this.dict) {
      return this.dict[this.next()];
    }
    if (x in this.protos) {
      throw Error(
        `Cannot use forward declaration '${x}' in calculated constant '${name}".`
      );
    }
    if (this.next() != "(") {
      throw Error(`Undefined constant '${x}'.`);
    }
    const value = this.parseCalc(name);
    if (this.next() != ")") {
      throw Error(`Expected ')' for calculated constant '${name}'.`);
    }
    return value;
  }
  ...
  private instruction(token: stringOrNumber): void {
    if (token == ":") {
      ...
      this.breakpoints[this.here()] = this.next() as string;
    } else if (token == ":proto") {
      this.next();
    } // deprecated.
    else if (token == ":alias") {
      this.aliases[
        this.checkName(this.next() as string, "alias")
      ] = this.register();
    } else if (token == ":const") {
      const name = this.checkName(this.next() as string, "constant");
      if (name in this.constants) {
        throw Error("The name '" + name + "' has already been defined.");
      }
      ...
      if (this.next() != "{") {
        throw Error("Expected '{' for definition of macro '" + name + "'.");
      }
      const body = [];
      let depth = 1;
      while (!this.end()) {
      ...
      this.macros[name] = {
        args: args,
        body: body as [string | number, number, number][]
      };
      ...
      for (let x = 0; x < macro.body.length; x++) {
        ...
        this.tokens.splice(x + this.currentToken, 0, value as [
          string | number,
          number,
          number
        ]);
      ...
      this.data(
        this.peek() == "{"
          ? this.parseCalculated("ANONYMOUS")
          : this.shortValue()
      );
    } else if (token == ":org") {
      this.hereaddr = this.constantValue();
    } else if (token == ";") {
      this.inst(0x00, 0xee);
      ...
      this.inst(0xf0 | this.register(), 0x33);
    } else if (token == "save") {
      const reg = this.register();
      ...
      this.expect(":=");
      this.inst(0xf0 | this.register(), 0x15);
    } else if (token == "buzzer") {
      this.expect(":=");
      this.inst(0xf0 | this.register(), 0x18);
    } else if (token == "if") {
      ...
      if (control[0] == "then") {
        ...
        this.branches.push([
          this.here(),
          this.pos as [string | number, number, number],
          "begin"
        ]);
        this.inst(0x00, 0x00);
      } else {
      ...
      this.immediate(0x00, this.wideValue());
      ...
      while (this.whiles[this.whiles.length - 1] !== null) {
        //TODO replaced != with !==. Is this ever undefined?
      ...
      if (plane > 3) {
        throw Error("the plane bitmask must be [0, 3].");
      }
      this.xo = true;
      ...
      this.xo = true;
      this.inst(0xf0, 0x02);
    } else if (token == "scroll-down") {
      this.schip = true;
      this.inst(0x00, 0xc0 | this.tinyValue());
    } else if (token == "scroll-up") {
      this.xo = true;
      this.inst(0x00, 0xd0 | this.tinyValue());
    } else if (token == "scroll-right") {
      this.schip = true;
      this.inst(0x00, 0xfb);
    } else if (token == "scroll-left") {
      this.schip = true;
      this.inst(0x00, 0xfc);
    } else if (token == "exit") {
      this.schip = true;
      ...
      if (flags > 7) {
        throw Error("saveflags argument must be v[0,7].");
      }
      this.schip = true;
      ...
      const flags = this.register();
      if (flags > 7) {
        throw Error("loadflags argument must be v[0,7].");
      }
      this.schip = true;
