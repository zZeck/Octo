...
const audioPatternEditor = textBox(
  document.getElementById("audio-pattern-editor")!,
  false,
  ""
);
const audioBlendEditor = textBox(
  document.getElementById("audio-blend-editor")!,
  false,
  ""
);
const audioToneEditor = textBox(
  document.getElementById("audio-tone-editor")!,
  true,
  ""
);
const audioPatternCanvas = document.getElementById(
  "audio-pattern-view"
) as HTMLCanvasElement;
const audioToneCanvas = document.getElementById(
  "audio-tone-view"
) as HTMLCanvasElement; // TODO correct?

const PATTERN_SIZE = 16;
const PATTERN_SCALE = 2;
const emptySound = range(PATTERN_SIZE).map((): number => 0);
function readPattern(source: Editor): number[] {
  return readBytes(source, PATTERN_SIZE);
}
function writePattern(target: Editor, bytes: number[]): void {
  return writeBytes(target, PATTERN_SIZE, bytes);
}
...
drawOnCanvas(
  audioPatternCanvas,
  (x: number, _y: number, draw: boolean): void => {
    // TODO unused?
    const index = Math.min(
      PATTERN_SIZE * 8,
      Math.max(0, Math.floor(x / PATTERN_SCALE))
    );
...
document.getElementById("audio-play")!.onclick = (): void => {
  if (audioSetup()) {
    ...
    document.getElementById("audio-error")!.innerHTML =
...
document.getElementById("audio-random")!.onclick = (): void => {
  writePattern(
    audioPatternEditor,
    emptySound.map((): number => (Math.random() * 256) & 0xff)
  );
  updateAudio();
};
...
document.getElementById("audio-left")!.onclick = (): void => {
  writePattern(
    audioPatternEditor,
    shiftBytes(readPattern(audioPatternEditor), 1)
  );
  updateAudio();
};
...
document.getElementById("audio-not")!.onclick = (): void => {
  writePattern(
    audioPatternEditor,
    readPattern(audioPatternEditor).map((a: number): number => ~a)
  );
  updateAudio();
};
