...
const sprite16 = toggleButton(
  document.getElementById("sprite-16")!,
  false,
  changeSpriteSize
); // TODO was 0 just being used for false?
const spriteColor = toggleButton(
  document.getElementById("sprite-color")!,
  false,
  updateSpriteEditor
); // TODO was 0 just being used for false?
const spriteClear = document.getElementById("sprite-clear")!;
const spritePalette = radioBar(
  document.getElementById("sprite-palette")!,
  1,
  (): void => {}
);
const spriteEditor = textBox(
  document.getElementById("sprite-editor")!,
  false,
  ""
);

spriteClear.onclick = (): void => {
  spritePixels = [];
  updateSpriteEditor();
};
function spriteLength(): number {
  return (sprite16.getValue() ? 32 : 15) * (spriteColor.getValue() ? 2 : 1);
}
function spriteDim(
  big: boolean
): {
...
function clampSpriteData(): void {
  for (let z = spriteLength() + 1; z < 64; z++) {
    spritePixels[z] = 0;
  }
}
...
function setSpritePixel(
  ...
  const t = spritePixel(x, y, wide);
  spritePixels[t.layer1] =
    (spritePixels[t.layer1] & ~t.mask) | (-Boolean(p & 1) & t.mask);
  spritePixels[t.layer2] =
    (spritePixels[t.layer2] & ~t.mask) | (-Boolean(p & 2) & t.mask);
}
...
function getSpritePixels(
  ...
  return range(dim.rows).map((row): number[] => {
    return range(dim.cols).map((col): number => {
      return getSpritePixel(
        mod(col - dx, dim.cols),
        mod(row - dy, dim.rows),
        dim.cols == 16,
        Number(c)
      ); // TODO construct correct?
    });
  });
}
function setSpritePixels(
  ...
  range(dim.rows).forEach((row): void => {
    range(dim.cols).forEach((col): void => {
      setSpritePixel(
        ...
        (pix[row] || [])[col] || 0
      ); // TODO value of c is ignored in function
    });
  });
}
...
document.getElementById("sprite-left")!.onclick = (): void =>
  scrollSprite(-1, 0);
document.getElementById("sprite-right")!.onclick = (): void =>
  scrollSprite(1, 0);
document.getElementById("sprite-up")!.onclick = (): void => scrollSprite(0, -1);
document.getElementById("sprite-down")!.onclick = (): void =>
  scrollSprite(0, 1);
...
export function updateSpriteEditor(): void {
  document
    .querySelectorAll<HTMLElement>("#sprite-palette>span")
    .forEach((x, i): void => {
