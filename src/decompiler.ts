...
const regNames = [
  //TODO clean this up. Probably should not be array at this point.
  "0",
  "1",
  "2",
  "3",
...
let reaching: {
  [address: number]: { [register: string]: { [reg: string]: boolean } };
} = {}; // map<address, map<register, set<int>>>
...
export function formatInstruction(a: number, nn: number): string {
  ...
  function name(map: { [address: number]: string }, nibbles: number): string {
    return nibbles in map ? map[nibbles] : hexFormat(nibbles);
  }

  if (a == 0x00 && y == 0xc) {
    return "scroll-down " + numericFormat(n);
  } // schip
  if (a == 0x00 && y == 0xd) {
    return "scroll-up " + numericFormat(n);
  } // xo-chip
  if (op == 0x00e0) {
    return "clear";
  }
  ...
  if (o == 0x1) {
    return "jump " + name(lnames, nnn);
  }
  if (o == 0x2) {
    return nnn in snames ? name(snames, nnn) : ":call " + hexFormat(nnn);
  }
  if (o == 0x3) {
    return "if " + vx + " != " + numericFormat(nn) + " then";
  }
  if (o == 0x4) {
    return "if " + vx + " == " + numericFormat(nn) + " then";
  }
  if (o == 0x5 && n == 0x0) {
    return "if " + vx + " != " + vy + " then";
  }
  if (o == 0x5 && n == 0x2) {
    return "save " + vx + " - " + vy;
  } // xo-chip
  ...
  if (o == 0xa) {
    return "i := " + name(lnames, nnn);
  }
  if (o == 0xb) {
    return "jump0 " + name(lnames, nnn);
  }
  if (o == 0xc) {
    return vx + " := random " + maskFormat(nn);
  } // TODO is this a bug? second parameter ignored? maskFormat(nn, 2)
  if (o == 0xd) {
    return "sprite " + vx + " " + vy + " " + numericFormat(n);
  }
  if (o == 0xe && nn == 0x9e) {
    return "if " + vx + " -key then";
  }
  if (o == 0xe && nn == 0xa1) {
    return "if " + vx + " key then";
  }
...
function formatNative(address: number, prefix: string): (string | number)[] {
  ...
  while (true) {
    ...
    if (typeof program[addr] === "undefined") {
      break;
    }
    if (typeof type[addr] !== "undefined") {
      break;
    }
    r += prefix;
    ...
    const h3 =
      hexFormat(op) + " " + hexFormat(a) + " " + hexFormat(b) + " # " + end;
    ...
    if (op == 0x3c) {
      addr += 2;
      r += h2 + "BN1  " + ha;
    } // branch if not flag 1
    if (op == 0x3d) {
      addr += 2;
      r += h2 + "BN2  " + ha;
    } // branch if not flag 2
    if (op == 0x3e) {
      addr += 2;
      r += h2 + "BN3  " + ha;
    } // branch if not flag 3
    if (op == 0x3f) {
      addr += 2;
      r += h2 + "BN4  " + ha;
    } // branch if not flag 4
    if (o == 0x4) {
      addr += 1;
      r += h1 + "LDA  " + hr;
    } // load and advance
    ...
    if (o == 0xa) {
      addr += 1;
      r += h1 + "PLO  " + hr;
    } // put in low byte of r
    if (o == 0xb) {
      addr += 1;
      r += h1 + "PHI  " + hr;
    } // put in high byte of r
    if (op == 0xc0) {
      addr += 3;
      r += h3 + "LBR  " + hb;
    } // lbranch always
    ...
    if (o == 0xd) {
      break;
    } // SEP is a context switch, effectively a return
  }
  return [addr - start, r + "\n"];
}

function copyReachingSet(source: {
  [register: string]: { [reg: string]: boolean };
}): {
  [registerName: string]: {
    [key: string]: boolean;
  };
...
function apply(
  ...
  function setType(typeAddress: number, typeDesired: string): void {
    ...
    if (desired == "code" && t == "smc") {
      return;
    }
    if (desired == "data" && t == "smc") {
      return;
    }
    if (desired == "code" && t == "data") {
      desired = "smc";
    }
    if (desired == "data" && t == "code") {
      desired = "smc";
    }
  ...
  if (op == 0xf000) {
    ...
    if (typeof labels[nnnn] === "undefined") {
      labels[nnnn] = [];
    }
    if (!labels[nnnn].includes(address)) {
      labels[nnnn].push(address);
    }
  }
  if (o == 0x1 || o == 0xa || o == 0xb) {
    if (typeof labels[nnn] === "undefined") {
      labels[nnn] = [];
    }
    if (!labels[nnn].includes(address)) {
      labels[nnn].push(address);
    }
  }
  if (o == 0x2) {
    if (typeof subroutines[nnn] === "undefined") {
      subroutines[nnn] = [];
    }
    if (!subroutines[nnn].includes(address)) {
      subroutines[nnn].push(address);
    }
  }
  if (o == 0x0 && x != 0x0) {
    if (typeof natives[nnn] === "undefined") {
      natives[nnn] = [];
    }
    if (!natives[nnn].includes(address)) {
      natives[nnn].push(address);
    }
  }
  ...
  function chaseReturns(): {
    ...
    for (const rsource of Object.keys(ret["rets"])) {
      for (const rdest of Object.keys(
        reaching[parseInt(rsource) - 2]["rets"]
      )) {
  ...
  if (SHIFT_QUIRKS && o == 0x8 && (n == 0x6 || n == 0xe)) {
    y = x;
  }

  if (op == 0x00ee) {
    ret["rets"] = chaseReturns();
  } // return
  if (o == 0x2) {
    ret["rets"] = single(address + 2);
  } // call
  if (o == 0x6) {
    ret[x] = single(nn);
  } // vx := nn
  if (o == 0x7) {
    ret[x] = unary(function(addr): number {
      return addr + nn;
    });
  } // vx += nn
  if (o == 0x8 && n == 0x0) {
    binary(function(_a, b): number {
      return b;
    });
  } // vx := vy //TODO fix _a?
  if (o == 0x8 && n == 0x1) {
    binary(function(addr, b): number {
      return addr | b;
    });
  } // vx |= vy
  if (o == 0x8 && n == 0x2) {
  ...
  if (o == 0x8 && n == 0x4) {
    bincarry(function(addr, b): number[] {
      return [addr + b, Number(addr + b > 0xff)];
    });
  } // TODO is this a bug?
  if (o == 0x8 && n == 0x5) {
    bincarry(function(addr, b): number[] {
      return [addr - b, Number(addr >= b)];
    });
  }
  if (o == 0x8 && n == 0x6) {
    bincarry(function(_a, b): number[] {
      return [b >> 1, b & 1];
    });
  } // TODO fix _a?
  if (o == 0x8 && n == 0x7) {
    bincarry(function(addr, b): number[] {
      return [b - addr, Number(b >= addr)];
    });
  }
  if (o == 0x8 && n == 0xe) {
    bincarry(function(_a, b): number[] {
  ...
  if (o == 0xf && nn == 0x29) {
    ret["i"] = unary(function(addr): number {
      return 5 * addr;
    });
  }
  if (o == 0xf && nn == 0x30) {
  ...
  if (o == 0xd) {
    ...
    if (n == 0) {
      markRead(color ? 63 : 31);
    } else {
      markRead(color ? (n - 1) * 2 : n - 1);
    }
  }
  if (o == 0xf && nn == 0x33) {
    // bcd
...
function successors(
  ...
  function preciseSkip(
    ...
    for (const vx of Object.keys(reaching[addr][x])) {
      ...
      for (const vy of Object.keys(reaching[addr][y])) {
        if (predicate(parseInt(vx), parseInt(vy), nn)) {
          skip = true;
        } else {
          pass = true;
        }
  ...
  if (o == 0x3) {
    return preciseSkip(address, function(xVal, _y, byte): boolean {
      return xVal == byte;
    });
  }
  if (o == 0x4) {
  ...
  if (o == 0x5) {
    return preciseSkip(address, function(xVal, yVal): boolean {
      return xVal == yVal;
    });
  }
  if (o == 0x9) {
...
export function analyzeInit(
  ...
  for (let x = 0; x < 4096 * 2; x++) {
    program[x] = 0x00;
  }
  for (let x = 0; x < rom.length; x++) {
    program[x + 0x200] = rom[x];
  }
}

export function analyzeWork(): boolean {
  function reachingMerge(
    a: { [key: string]: { [key: string]: boolean } },
    b: { [key: string]: { [key: string]: boolean } }
  ): boolean {
...
export function analyzeFinish(): void {
  ...
  let ssize = 0;
  let nsize = 0;
  if (typeof labels[0x200] === "undefined") {
    labels[0x200] = [4097];
  }
  lnames[0x200] = "main";
  snames[0x200] = "main";
  for (let x = 0; x < program.length; x++) {
    ...
    if (x in labels) {
      lnames[x] = `label-${lsize++}`;
    }
    if (x in subroutines) {
      snames[x] = `sub-${ssize++}`;
    }
    if (x in natives) {
      nnames[x] = `machine-${nsize++}`;
    }
  }
}
...
export function formatProgram(programSize: number): string {
  ...
  if (VF_ORDER_QUIRKS) {
    ret +=
      "# analyzed with arithmetic results written to vf after the status flag.\n";
  }
  ret += "\n";
  ...
  function findOutside(
    source: { [key: number]: number[] },
    dest: { [key: number]: boolean },
  ...
  for (let x = 0; x < programSize; x++) {
    ...
    if (a in labels) {
      if (lnames[a] == "main" && !(a in subroutines)) {
        ret += ": main\n";
      }
      ...
      for (let z = 0; z < labels[a].length; z++) {
        ...
        for (let scan = a; scan <= u; scan += 2) {
          ...
          if ((program[scan] & 0xf0) == 0x10) {
            ...
            const target =
              ((program[scan] & 0xf) << 8) | (program[scan + 1] & 0xff);
            if (target < a) {
              foundBreak = true;
              break;
            }
    ...
    if (pendingAgain.includes(a)) {
      ...
      if (program[a] == 0xf0 && program[a + 1] == 0x00) {
        ...
        ret +=
          indent +
          formatInstruction(program[a], program[a + 1]) +
          singleResult(a) +
          "\n";
        x++;
      }
