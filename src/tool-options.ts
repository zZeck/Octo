...
// TODO fix | 'none' hack?
const compatProfile = radioBar<CompatibilityProfileFlags | "none">(
  document.getElementById("compatibility-profile")!,
  CompatibilityProfileFlags.octo,
  setCompatibilityProfile
);
const screenRotation = radioBar<number>(
  document.getElementById("screen-rotation")!,
  0,
  (x: number): void => {
    emulator.screenRotation = Number(x);
  }
);
...
const compatibilityFlags: flagFunctions = {
  shiftQuirks: checkBox(
    document.getElementById("compat-shift")!,
    false,
    setOptions
  ),
  loadStoreQuirks: checkBox(
    document.getElementById("compat-load")!,
    false,
    setOptions
  ),
  vfOrderQuirks: checkBox(
    document.getElementById("compat-vf")!,
    false,
    setOptions
  ),
  clipQuirks: checkBox(
    document.getElementById("compat-clip")!,
    false,
    setOptions
  ),
  jumpQuirks: checkBox(
    document.getElementById("compat-jump0")!,
    false,
    setOptions
  ),
  vBlankQuirks: checkBox(
    document.getElementById("compat-vblank")!,
    false,
    setOptions
  ),
  maxSize: radioBar<number>(
...
function setCompatibilityProfile(x: CompatibilityProfileFlags | "none"): void {
  ...
  for (const key of Object.keys(compatibilityFlags))
    emulator[key] = p[key as CampatibilityFlags];
  saveLocalOptions();
  updateOptions();
}
...
function setOptions(): void {
  for (const key of Object.keys(compatibilityFlags))
    emulator[key] = compatibilityFlags[key as CampatibilityFlags].getValue();
  saveLocalOptions();
  updateOptions();
}
// TODO edited loops to be const key. correct?
export function updateOptions(): void {
  for (const key of Object.keys(compatibilityFlags))
    compatibilityFlags[key as CampatibilityFlags].setValue(emulator[key]);
  ...
  for (const key of Object.keys(compatibilityProfiles)) {
    ...
    const same = Object.keys(p).every(
      (x: string): boolean => emulator[x] == p[x as CampatibilityFlags]
    );
