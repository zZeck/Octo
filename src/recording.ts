...
export function gifBuilder(
  ...
  const t = (x: string): void =>
    x.split("").forEach((y: string): number => b(y.charCodeAt(0)));
  ...
  return {
    ...
    frame: (pixels: Uint8Array, delay?: number): void => {
      // TODO delay undefined?  Uint8Array correct?
...
function paletteToRGB(pal: string[]): number[] {
  ...
  return pal.map(
    (_, i): number =>
      (d.data[i * 4] << 16) | (d.data[i * 4 + 1] << 8) | d.data[i * 4 + 2]
  );
}
...
export function recordFrame(): void {
  ...
  const last = (document.getElementById(renderTarget) as CanvasWithLastFrame)
    .last;
  if (
    last != undefined &&
    arrayEqual(last.p[0], emulator.p[0]) &&
    arrayEqual(last.p[1], emulator.p[1])
  ) {
    heldTicks++;
  } else {
    if (heldFrame !== null) currentRecording.frame(heldFrame, heldTicks * 2);
    if (emulator.hires) {
      heldFrame = new Uint8Array(
        zip(
          emulator.p[0].slice(0, 128 * 64),
          emulator.p[1],
          (a: number, b: number): number => a | (b << 1)
        )
      ); // TODO correct construct call?
    } else {
      heldFrame = new Uint8Array(
        range(128 * 64).map((x: number): number => {
...
runRecord.onclick = (): void => {
  if (currentRecording === null) {
    ...
    (document.getElementById(
      renderTarget
    ) as CanvasWithLastFrame).last = undefined; // flush repaint buffer
  } else {
    if (heldFrame !== null) currentRecording.frame(heldFrame, heldTicks * 2); //TODO strict null
    saveAs(
      new Blob([new Uint8Array(currentRecording.finish())], {
        type: "image/gif"
      }),
      "recording.gif"
    );
