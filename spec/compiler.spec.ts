...
describe("compiler", (): void => {
  ...
  it("cave explorer compiles correctly", async (): Promise<void> => {
    const caveExplorer = await (await fetch(
      "/base/examples/caveexplorer.8o"
    )).text();
    ...
    const romHexStr = compiler.rom.reduce(
      (acc, cur): string => acc + cur.toString(16).padStart(2, "0"),
      ""
    );
